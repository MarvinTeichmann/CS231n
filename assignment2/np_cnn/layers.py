...
def affine_forward(x, w, b):
    ...
    ##########################################################################
    num = x.shape[0]
    bigd = np.prod(x.shape[1:])
    x2 = np.reshape(x, (num, bigd))
    out = np.dot(x2, w) + b
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def affine_backward(dout, cache):
    ...
    bign = x.shape[0]
    bigd = np.prod(x.shape[1:])
    x2 = np.reshape(x, (bign, bigd))

    dx2 = np.dot(dout, w.T)  # N x D
    dw = np.dot(x2.T, dout)  # D x M
    db = np.dot(dout.T, np.ones(bign))  # M x 1
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        x_pad = np.pad(
            x[n, :, :, :], ((0, 0), (pad, pad), (pad, pad)), 'constant')
        for f in range(F):
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                    ...
                    out[n, f, h_prime, w_prime] = np.sum(
                        window * w[f, :, :, :]) + b[f]
    ##########################################################################
    #                             END OF YOUR CODE                              #
    ##########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        dx_pad = np.pad(
            dx[n, :, :, :], ((0, 0), (pad, pad), (pad, pad)), 'constant')
        x_pad = np.pad(
            x[n, :, :, :], ((0, 0), (pad, pad), (pad, pad)), 'constant')
        for f in range(F):
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                    ...
                    w1 = w_prime * stride
                    w2 = w_prime * stride + WW
                    dx_pad[:, h1:h2, w1:w2] += w[f, :, :, :] * \
                        dout[n, f, h_prime, w_prime]
                    dw[f, :, :, :] += x_pad[:, h1:h2, w1:w2] * \
                        dout[n, f, h_prime, w_prime]
    ...
    for n in range(N):
        for h in range(H_prime):
            for w in range(W_prime):
                ...
                out[n, :, h, w] = np.max(window.reshape(
                    (C, pool_height*pool_width)), axis=1)
    ##########################################################################
    #                             END OF YOUR CODE                              #
    ##########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for h in range(H_prime):
                for w in range(W_prime):
                    ...
                    dx[n, c, h1:h2, w1:w2] = np.reshape(
                        window3, (pool_height, pool_width)) * dout[n, c, h, w]
    ##########################################################################
    #                             END OF YOUR CODE                              #
    ##########################################################################
    return dx
